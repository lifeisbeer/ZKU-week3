...
const encrypt = async (
  ...
  // Generate the IV
  const iv = buf2Bigint(mimc7.multiHash(plaintext, BigInt(0))); // make everything Bigint

  const ciphertext: Ciphertext = {
    iv,
    data: plaintext.map((e: bigint, i: number): bigint => {
      return BigInt(e) + buf2Bigint(mimc7.hash(sharedKey, iv + BigInt(i))) // make everything Bigint
    })
  }
...
const decrypt = async (
  ...
  const plaintext: Plaintext = ciphertext.data.map(
    (e: bigint, i: number): bigint => {
        return BigInt(e) - buf2Bigint(mimc7.hash(sharedKey, ciphertext.iv + BigInt(i))) // make everything Bigint
    }
  )
